...
function submit(){
  ...
  case "$executor" in
    slurm)
          ...
          CLUSTER_OPTS="sbatch --gres {resources.gres} --cpus-per-task {threads} -p {resources.partition} -t {resources.time} --mem {resources.mem} --job-name={params.rname} -e $SLURM_DIR/slurm-%j_{params.rname}.out -o $SLURM_DIR/slurm-%j_{params.rname}.out"
          ...
          if [[ ${6#\'} != /lscratch* ]]; then
            CLUSTER_OPTS="sbatch --cpus-per-task {threads} -p {resources.partition} -t {resources.time} --mem {resources.mem} --job-name={params.rname} -e $SLURM_DIR/slurm-%j_{params.rname}.out -o $SLURM_DIR/slurm-%j_{params.rname}.out"
          fi
